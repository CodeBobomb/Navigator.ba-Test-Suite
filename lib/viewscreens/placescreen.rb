...
module ViewScreen
	module PlaceScreen
		class << self
			...
			def gallery_nav_right		 
				 @@arrow_right.click
			end

			def gallery_nav_left
				@@arrow_left.click
			end
			...
			def details_exist?
				...
				return !(@name.nil? || @image.nil? || @address.nil? || @phone.nil? || @email.nil? || @rating.nil? || @ratnum.nil? || @worktime.nil? || @web.nil? || @desc.nil?)
			end
		end
	end
end